...
static SemaphoreHandle_t i2c_mux[I2C_NUM_MAX];
static bool i2c_initialized[I2C_NUM_MAX];
...
bool i2c_init(i2c_port_t i2c_port, int clk_speed, int sda_pin, int scl_pin)
{
    ...
    LOGI("Initialized i2c port %d (%d), sda: %d, scl: %d", i2c_port, clk_speed, sda_pin, scl_pin);
    return true;
}
